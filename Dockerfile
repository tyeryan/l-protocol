...
RUN mkdir -p /tmp/grpc-go && \
    curl -L https://github.com/grpc/grpc-go/archive/v1.58.2.zip > /tmp/grpc-go/grpc-go.zip && \
    cd /tmp/grpc-go && \
    unzip grpc-go.zip && \
    mkdir -p /go/src/google.golang.org/grpc/ && \
    cp -r /tmp/grpc-go/grpc-go-1.58.2/* /go/src/google.golang.org/grpc/

# Install protoc-gen-go
RUN mkdir -p /tmp/protobuf && \
    curl -L https://github.com/golang/protobuf/archive/refs/tags/v1.5.3.zip > /tmp/protobuf/protobuf.zip && \
    cd /tmp/protobuf && \
    unzip protobuf.zip && \
    mkdir -p /go/src/github.com/golang/protobuf/ && \
    cp -r /tmp/protobuf/protobuf-1.5.3/* /go/src/github.com/golang/protobuf/ && \
